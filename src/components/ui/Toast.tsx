...
const ToastComponent: React.FC<{ toast: Toast }> = ({ toast }) => {
  ...
  return (
    <div
      className={cn(
        'relative flex items-start p-4 rounded-lg border shadow-lg pointer-events-auto',
      ...
      aria-atomic="true"
    >
      <div className={cn('flex-shrink-0 mt-0.5', iconStyles[toast.variant])}>
        {renderIcon()}
      </div>
      ...
      {toast.dismissible && (
        <button
          ...
          className="ml-4 flex-shrink-0 rounded-md focus:outline-none focus:ring-2 focus:ring-offset-2 p-1 hover:opacity-70 transition-opacity"
